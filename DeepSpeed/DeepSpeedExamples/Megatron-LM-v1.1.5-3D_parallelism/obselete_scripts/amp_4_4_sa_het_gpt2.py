...
exp_name = f"sa_het_gpt2_p3_domino"
...
h_w, micro_bs_list, configs, known = generate_initial(M, N, global_bs, threads=num_threads, domino=True)
...
for i in tqdm(range(num_iter)):
    ...
    for j in range(num_threads):
        ...
        step = neighbor((h, w, mbs), global_bs, known, M, N, domino=True)
        if step is None:
            stop_index.append(j)
            with open(record_file, "a") as fp:
