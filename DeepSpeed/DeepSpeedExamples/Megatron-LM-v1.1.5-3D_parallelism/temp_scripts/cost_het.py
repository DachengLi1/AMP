    ...
    def init_param(self):
        ...
        if self.estimate:
            for mp_size in [1,2,4]:
                ...
                cur_cost1 = 3 * np.load(f"{known_record}.npy")
                known_record = f"known_cost/{self.model_type}_G4_{mp_size}"
                cur_cost2 = 3 * np.load(f"{known_record}.npy")

                # average between different speed of GPUs
                cur_cost = cur_cost1 * 0.75 + cur_cost2 * 0.25 
    ...
    if model_type == "gpt2":
        ...
        #    mp_avg /= pp
            ...
            for layer_id in range(_num_layer):
                ...
                cur_layer = orig_bs * cost_single[str(int(mp.item()))][layer_id]# / mp.item()
