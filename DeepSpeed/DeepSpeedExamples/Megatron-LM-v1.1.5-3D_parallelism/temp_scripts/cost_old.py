...
from pipe import pipe_ds, pipe_dp, pipe_cost
...
class AMP(nn.Module):
    ...
    def init_param(self):
        ...
        if self.estimate:
            ...
            # forward + backward
            self.cost_single = 3 * np.load(f"{record_path}.npy")
...
def get_cost_c(cluster_info, model_config, parallel_config, amp_config, dp_index=0, model_type="GPT2"):
    ...
    if model_type == "gpt2":
      ...
      for i in range(_num_layer):
        ...
        _num_layer = 32 + depth[0] + depth[1] + depth[2] + depth[3] + depth[4] + depth[5]

        layer_volume = [bs * h * bottom ** 2] * _num_layer
        #for kkk in range(depth[5]):
        #    layer_volume.append(bs * h * 64 * 2)
        #
        #_num_layer += depth[5]
        # for _ in range(5):
        #     layer_volume.append(bs * 64 * h)
        # for _ in range(4):
        #     layer_volume.append(bs * 64 * h)
        # for _ in range(4):
        #     layer_volume.append(bs * 64 * h)
...
def get_cost_e(cluster_info, model_config, parallel_config, amp_config, model_type):    
    ...
    if model_type == "gpt2":
        ...
        cost_e = torch.zeros((int(dp.item()), _num_layer))
        # TODO: loop through all dp, and get the average
        print("sadcsaca")
        for i in range(int(dp.item())):
                ...
                for k in range(int(mp.item())):
                    ...
                    if first_node is None:
                        ...
                        if node_cur == first_node:
                            mp_avg += 2 / (mp * cluster_info[node_cur][1])
                        else:
                            mp_avg += 2 / (mp * min(cluster_info[node_cur][0], cluster_info[first_node][0]))
            mp_avg /= pp
            for layer_id in range(_num_layer):
                layer_type = _layer[layer_id]
                
                if layer_type == "embed2h":
                    cur_layer = alpha *  bs * s * h * mp_avg 
                elif layer_type == "embed2v":
                    cur_layer = 6 * bs * s * h * v / mp
                    cur_layer += alpha * v * h / mp * mp_avg
                elif layer_type == "transformer_layer":
                    cur_layer = (72 * bs * s * h ** 2 + 12 * bs * s ** 2 * h) / mp
                    cur_layer += alpha * (7*h**2/mp + 2*bs*s*h) * mp_avg
                elif layer_type == "noop":
                    cur_layer = 0
                else:
                    raise RuntimeError("Unknown layer type.")
        ...
        # Compute the constant along the pipeline: 2*(N-1)/(NB)
            ...
            for inc in range(depth[0]):
                ...
                cur_layer = orig_bs * (cost_single[cur_layer_id] / (mp).item())
                #print(cur_layer)
...
def predict_single(config, bs, mbs, cluster_info, model_config, amp_config, oth, model_type):
    ...
    cost += dp_side_cost
    print(ds_partition, dp_side_cost)
