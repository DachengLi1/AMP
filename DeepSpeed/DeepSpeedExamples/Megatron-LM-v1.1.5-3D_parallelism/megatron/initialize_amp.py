...
def _initialize_distributed():
    ...
    if torch.distributed.is_initialized():
        assert False, "torch distributed already initialized"
        ...
        if device_count > 0:
            assert args.local_rank is not None, "local rank not given"
        ...
        print(f"initializing using rank {args.rank} and world_size {args.world_size}")
    ...
    # sanity check we have disabled the original outside
    # assert args.pipe_parallel_size < 0
    # assert args.model_parallel_size < 0
    # pp = args.pipe_parallel_size
    # mp = args.model_parallel_size
    # assert args.world_size % (pp * mp) == 0
    # dp = args.world_size // (pp * mp)

    pp = args.amp_pipe_parallel_size
    mp = args.amp_model_parallel_size
    dp = args.world_size // (pp * mp)

    assert pp is not None and mp is not None

    # from deepspeed.runtime.pipe.topology import AmpTopology #PipeModelDataParallelTopology
    from deepspeed.runtime.pipe.topology import PipeModelDataParallelTopology
    # query how to setup 3D Topology
