...
from megatron import get_elap, reset_elap
...
def pretrain(train_valid_test_dataset_provider, model_provider,
    ...
    args = get_args()
    timers = get_timers()
    print("finish megatron initialize")
...
def train_step_pipe(model, data_iterator):
    ...
    args = get_args()
    timers = get_timers()
    print("using training step pipe")
...
def training_log(loss_dict, total_loss_dict, learning_rate, iteration,
    ...
    elap_list = get_elap()
    elap_list.append(elapsed_time)
    if iteration % args.log_interval == 0:
        elap_arr = np.asarray(elap_list)
        elapsed_time = np.sum(elap_arr)
        reset_elap()
        if writer and torch.distributed.get_rank() == 0:
        ...
        log_string += '  mean (ms): {:.1f} std (ms): {:.1f}|'.format(
           np.mean(elap_arr*1000.0), np.std(elap_arr*1000.0))
...
def build_train_valid_test_data_iterators(
    ...
    if mpu.get_model_parallel_rank() == 0 and pipe_load:
        ...
        print("------batch size:",args.batch_size, data_parallel_size, args.gas)
