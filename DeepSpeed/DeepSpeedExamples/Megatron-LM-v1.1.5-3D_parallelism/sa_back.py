...
class candidate():
    ...
    def __eq__(self, obj):
        ...
        if (self.rank_map != obj.rank_map).any():
            return False
...
def generate_initial(M, N, gbs, threads):
    h_w_list = []
    ...
    for (h, w) in h_w_list:
        ...
        for sol in solution:
            solution_mbs.append((sol, [], remain))
 
        config_idx = np.random.choice(len(solution_mbs), size=1)[0]
        config = solution_mbs[config_idx][0]
        configs.append(config)
        
        #solution.pop(config_idx)
        solution_mbs[config_idx][1].append(1)
        
        # edge case: only mbs=1 is possible for this config
        all_possible = factor(remain)
        if len(all_possible) == 1:
            assert all_possible[0] == 1
...
def neighbor(cur, gbs, known, M, N, timeout = 1, verbose=True):
    h, w, mbs = cur
    #print("cur:", h, w, mbs)
    #print(known)
    ...
    while len(feasible_index) > 0:
       ...
       # print(index)
        ...
        #        print(f"0: {i}, {w}")
            ...
            if len(valid) == 0:
      #          print("0 no valid solution", solution_mbs, known)
     ...
     #       print("exp:", known, known[(new_h, w)], known[(new_h, w)][new_config_idx], known[(new_h, w)][new_config_idx][1])
            explored_mbs = set(known[(new_h, w)][new_config_idx][1])
            possible_mbs = set(factor(remain, upper=known[(new_h, w)][new_config_idx][2]))
            unexplored_mbs = possible_mbs.difference(explored_mbs)
    #        print((new_h,w), unexplored_mbs, explored_mbs, possible_mbs)
            assert len(unexplored_mbs) > 0
            
            new_mbs = np.random.choice(list(unexplored_mbs), size=1)[0]
            known[(new_h,w)][new_config_idx][1].append(new_mbs)
       #     print(f"debug conf: {known} {new_h} {w} {new_config_idx} {new_mbs} {explored_mbs} {unexplored_mbs}, {known}")
            
            if len(unexplored_mbs) == 1:
        #        print(f"poping: {known}")
                known[(new_h, w)].pop(new_config_idx)
      
   #         print("ret", new_h, w, new_mbs, ret)
         ...
         #       print(f"1: {h}, {i}")
            ...
            if len(valid) == 0:
  #              print("1 no valid solution", solution_mbs, known)
 ...
 #           print("exp:", known, known[(h, new_w)], known[(h, new_w)][new_config_idx], known[(h, new_w)][new_config_idx][1])
            explored_mbs = set(known[(h, new_w)][new_config_idx][1])
            possible_mbs = set(factor(remain, upper=known[(h, new_w)][new_config_idx][2]))
            unexplored_mbs = possible_mbs.difference(explored_mbs)

#            print((h,new_w), unexplored_mbs, explored_mbs, possible_mbs)
            assert len(unexplored_mbs) > 0
            new_mbs = np.random.choice(list(unexplored_mbs), size=1)[0]
            known[(h,new_w)][new_config_idx][1].append(new_mbs)
           # print(f"debug conf: {known} {h} {new_w} {new_config_idx} {new_mbs} {explored_mbs} {unexplored_mbs}, {known}")

            if len(unexplored_mbs) == 1:
                print(f"poping: {known}")
                known[(h, new_w)].pop(new_config_idx)

#            print("ret", h, new_w, new_mbs, ret)
...
def random_strategy(M, N, gbs, known):
    if known is None:
        known = dict()
