...
from pipe import pipe_ds, pipe_dp, pipe_gpt2, pipe_cost
...
def get_cost_e(cluster_info, model_config, parallel_config, amp_config, model_type):    
    ...
    if model_type == "gpt2":
        ...
        for i in range(int(dp.item())):
            ...
            mp_avg /= pp
            assert _num_layer == len(cost_single), "predicted number of layers not equal to actual"
            mp_total = 0
            mp_total_volume = 0
            for layer_id in range(_num_layer):
                layer_type = _layer[layer_id]
                cur_layer = orig_bs * cost_single[layer_id] / (mp).item()
